...
const HeroStep = () => {
  ...
  // --- FIX 2: Updated uploadCroppedImage function (Resizes & uses JPEG) ---
  const uploadCroppedImage = async () => {
    if (!imgRef.current || !canvasRef.current || !crop) return;
    ...
    const MAX_DIMENSION = 2000; // Max output size (2000px)
    let canvasWidth = pixelCrop.width;
    let canvasHeight = pixelCrop.height;

    // Rescale if cropped image is too large
    ...
    // Draw the resized, cropped image onto the hidden canvas
    const ctx = canvas.getContext('2d');
    if (!ctx) {
    ...
    canvas.toBlob(async (blob) => {
      ...
      setError('');
      const formData = new FormData();
      formData.append('file', blob, `profile_${Date.now()}.jpg`); // <-- Use .jpg
      formData.append('upload_preset', CLOUDINARY_UPLOAD_PRESET);
    ...
    }, 'image/jpeg', 0.85); // <-- Change to image/jpeg and 85% quality
  };
  // --- END OF FIX 2 ---
    ...
    <div className="space-y-6">
      ...
      {showCropper && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          ...
          {/* --- FIX 1: Modal size increased (max-w-xl -> max-w-3xl) --- */}
          <div className="bg-white p-6 rounded-lg max-w-3xl w-full"> 
            
            <h4 className="text-xl font-semibold mb-4">Crop Your Image</h4>
            {error && <p className="text-red-600 text-sm mb-2">{error}</p>}
            
            {/* --- FIX 1: Crop area height increased (max-h-[60vh] -> max-h-[80vh]) --- */}
            <div className="max-h-[80vh] overflow-auto mb-4 bg-gray-100 p-2 border rounded"> 
              <ReactCrop
                crop={crop}
              ...
              >
                <img
                  ...
                  src={imgSrc}
                  alt="Upload preview"
                  // --- FIX 1: Image max-height increased (70vh -> 80vh) ---
